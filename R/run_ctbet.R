...
run_ctbet = function(file_nifti, file_mask) {
  ...
  file.copy(file_nifti, full_image_folder)
  on.exit(unlink(full_image_folder, recursive = TRUE))
  ...
  on.exit(unlink(full_save_folder, recursive = TRUE), add = TRUE)
  ...
  outfile = file.path(full_save_folder, basename(file_nifti))
  file.copy(outfile, file_mask)
  file_mask
}
